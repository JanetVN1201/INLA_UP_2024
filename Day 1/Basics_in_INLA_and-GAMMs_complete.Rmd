---
title: "INLA for environmental sciences - Day 1"
...
```{r Example 1.d}
#INLA model
model3.1 <- inla(status ~ 1 + age + sex + year + thickness + ulcer,
                 family = "binomial",
                 data = melanoma)
```
...
```{r Example 2.d}
result1_sids <- inla(SID74 ~ 1 + nwp_hat74,
                     family = "poisson",
                     E = E74,
                     data = nc.sids,
                     control.predictor = list(compute = TRUE))
...
```{r Example 2.g}
result2_sids <- inla(SID74 ~ 1 + nwp_hat74 + 
                       f(CNTY.ID, model = "iid",
                      hyper = list(theta = list(param = c(2,2)))),
                     family = "poisson",
                     E = E74,
                     data = nc.sids,
                     control.predictor = list(compute = TRUE))
...
result2_sids$summary.random$CNTY.ID
plot(result2_sids$marginals.random$CNTY.ID$index.1, type = "l")
...
```{r Example 3.d}
inla.doc("seasonal")
airp.seasonal <- inla(l_airp ~ -1 + year + f(ID, model = "seasonal", season.length = 12),
                 family = "gaussian",
                 data = airp.data,
                 control.predictor = list(compute = TRUE),
                 control.family = list(hyper = list(prec = list(param = c(1,0.2)))))
...
```{r Example 3.e}
inla.doc("rw")
airp.rw1 <- inla(l_airp ~ -1 + year + f(ID, model = "rw1", scale.model = T),
                 family = "gaussian",
                 data = airp.data,
                 control.predictor = list(compute = TRUE),
                 control.family = list(hyper = list(prec = list(param = c(1,0.2)))))
...
airp.rw2 <- inla(l_airp ~ -1 + year + f(ID, model = "rw2", scale.model = T),
                 family = "gaussian",
                 data = airp.data,
                 control.predictor = list(compute = TRUE),
                 control.family = list(hyper = list(prec = list(param = c(1,0.2)))))
